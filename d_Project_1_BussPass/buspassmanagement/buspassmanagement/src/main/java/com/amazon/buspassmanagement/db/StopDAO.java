...
public class StopDAO implements DAO<Stop>{
	...
	public int update(Stop object) {
		String sql = "UPDATE Stops set address = '"+object.address+"', sequenceOrder = "+object.sequenceOrder+", routeId = "+object.routeId+" WHERE id = "+object.id;
		return db.executeSQL(sql);
	}

	@Override
	public int delete(Stop object) {
		String sql = "DELETE from Stops WHERE id = "+object.id;
		return db.executeSQL(sql);
	}
